...
var PowerRoomModule = {
    ...
    addPhoto: function(roomId, photoData) {
        ...
        // 转换为黑白后存储
        this.toGrayImage(photoData, function(grayPhoto) {
            self.data[roomId].photos.push(grayPhoto);
            self.saveData();
            self.render();
        });
    },
    ...
    generatePDF: function() {
        ...
        try {
            ...
            for (var i = 0; i < summary.length; i++) {
                ...
                for (var j = 0; j < item.inspectedList.length; j++) {
                    ...
                    for (var k = 0; k < roomData.photos.length; k++) {
                        ...
                        try {
                            // 转换为黑白图片
                            var grayImg = self.toGraySync(roomData.photos[k]);
                            pdf.addImage(grayImg, 'JPEG', margin, margin + 10, pageWidth - margin * 2, pageHeight - margin * 2 - 10);
                        } catch(e) {
                            pdf.setFontSize(10);
                            pdf.setFont('helvetica', 'normal');
            ...
            pdf.save('强电间巡检_' + this.formatDate(new Date()) + '.pdf');
        } catch(e) {
