...
const CapacitorModule = {
    ...
    LOCATIONS: [
        ...
        { id: 17, name: '商业六 Tx-R(S)7-2', row: 21 },
        { id: 18, name: '商业六 Tx-R(S)7-3', row: 22 },
        { id: 19, name: '商业六 Tx-R(S)8-2', row: 23 },
        { id: 20, name: '商业六 Tx-R(S)8-3', row: 24 },
    ...
    parseUrlParams() {
        ...
        console.log('CapacitorModule context:', this.context);
    },
    ...
    renderCheckItems(locId) {
        ...
        console.log('renderCheckItems for locId:', locId, 'hasPhoto:', hasPhoto, 'photo length:', this.locationPhotos[locId] ? this.locationPhotos[locId].length : 'none');
        ...
        html += `<div class="photo-section" onclick="event.stopPropagation()">
            ...
            <div class="photo-upload-area ${hasPhoto ? 'has-photo' : ''}" id="photo-area-${locId}" onclick="CapacitorModule.triggerPhotoUpload(event, ${locId})">
                ...
                ${hasPhoto ? `<img src="data:image/jpeg;base64,${this.locationPhotos[locId]}" class="photo-preview" alt="热成像照片">` : ''}
            </div>
        </div>`;
    ...
    handlePhoto(input, locId) {
        // 已废弃，使用 triggerPhotoUpload + window.parent.requestFileChoose 代替
    },
    ...
    async generateExcel() {
        ...
        try {
            ...
            const buffer = await new Promise(function(resolve, reject) {
                ...
                xhr.open('GET', '../../templates/capacitor_template_new.xlsx', true);
            ...
            const setCellValue = (doc, cellRef, value) => {
                ...
                if (!cell) return;
                cell.setAttribute('t', 'str');  // 设为内联字符串类型
                    ...
                    if (selfRef.context.date) {
                        try {
                            var reportData = {
                                ...
                                content: '电容检查及测温 - ' + selfRef.context.date,
                                executor: document.getElementById('inspector')?.value || '',
