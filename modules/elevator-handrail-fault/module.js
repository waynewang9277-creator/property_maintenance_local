...
const FaultModule = {
    ...
    async generateExcel() {
        ...
        try {
            ...
            var response = await fetch('../../assets/templates/elevator_monthly_report.xlsx');
            ...
            link.download = '电梯月度故障报告_' + month.split('-')[1] + '月.xlsx';
            link.href = url;
            link.click();
    ...
    renderStatList: function() {
        ...
        async generateStatExcel() {
            ...
            try {
                ...
                var response = await fetch('../../assets/templates/elevator_monthly_stat.xlsx');
                var buffer = await response.arrayBuffer();
                var zip = await JSZip.loadAsync(buffer);
                ...
                var url = URL.createObjectURL(blob);
                var link = document.createElement('a');
                link.download = '电梯月度故障统计_' + month.split('-')[1] + '月.xlsx';
                link.href = url;
                link.click();
                URL.revokeObjectURL(url);
